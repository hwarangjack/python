...
# Middle Name 수정
for i in os.listdir('.'):
    if i.startswith(yyyymm) and (i.endswith('.xls') or i.endswith('.xlsx')):
        try:
            j=i.split(' ')
...
# # Last Name 수정
for i in os.listdir('.'):
    if i.startswith(yyyymm) and (i.endswith('rjsrkd.xls') or i.endswith('rjsrkd.xlsx')):
        try:
    ...
    if i.startswith(yyyymm) and (i.endswith('dusrma.xls') or i.endswith('dusrma.xlsx')):
        try:

...
# now=pyg.position()
# pyg.screenshot('step14.png',region=(now.x,now.y,120,20))
...
#로그인
one=pyg.locateCenterOnScreen('step1.png')
pyg.click(one.x,one.y)
...
for i in a:
    try:
        ...
        pyg.press('enter')
        time.sleep(1*time_interval)
        pyg.click(five.x-210,five.y+25,2)
        time.sleep(1.5*time_interval)
